...
description: jpzip の Go / TypeScript / Python / Rust / Ruby / Dart / PHP / Swift SDK を Claude Code と 6 時間で実装した話。プロトコル先行設計、翻訳タスクとしての多言語移植、言語固有の落とし穴。
lang: ja
...
> シリーズ最終回。[jpzip](https://jpzip.nadai.dev/) の SDK 8 言語版（Go / TypeScript / Python / Rust / Ruby / Dart / PHP / Swift）を、Claude Code を相棒に 6 時間で書いた話です。先に[1 本目](https://jpzip.nadai.dev/blog/0001-cloudflare-pages-micro-saas/)・[2 本目](https://jpzip.nadai.dev/blog/0002-cloudflare-pages-static-zipcode-delivery/)・[3 本目](https://jpzip.nadai.dev/blog/0003-mcp-server-japanese-postcode/)を読むとこの記事の数字が腑に落ちます。
...
- 鍵は <strong>「先にプロトコルを文章で固める」</strong>こと。実装は AI に任せられる粒度まで落とす
- 言語固有の落とし穴は確実にあるので、**「言語ごとの慣習に翻訳してもらう」プロンプトに切り替える**

## 「8 言語 6 時間」の正体

最初に正直に書いておきます。「6 時間で 8 言語の SDK を書いた」という言い方は、**Go SDK の開発開始から 8 言語ぶんの初回 publish までの時間**を指しています。タイマーをスタートさせる前に、こうした準備が積み上がっていました:

- データセット（120,677 件の JSON）が CDN に公開済み（[第 2 回参照](https://jpzip.nadai.dev/blog/0002-cloudflare-pages-static-zipcode-delivery/)）
- プロトコル仕様 `spec/v1/protocol.md` が JSON Schema 込みで固定済み

つまり「**仕様と CDN が揃った状態から、Go を参照実装として 8 言語ぶんを一気に書いて publish した**」のが 6 時間です。これは AI 駆動開発でできた、というより、**AI 駆動開発のためにできる前提を全部揃えてから始めた**結果です。
...
最初の Go SDK を書いた時点で参照実装が手に入ります。残り 7 言語は、Claude Code への指示を **「翻訳タスク」として組み立てる**のが効きました。
